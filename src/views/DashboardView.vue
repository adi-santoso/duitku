          ...
          <div class="space-y-2.5">
            <div v-for="b in budgetSummary.slice(0, 3)" :key="b.id">
              <div class="flex items-center justify-between mb-1">
                ...
                <span class="text-xs font-semibold" :class="b.spent > b.amount ? 'text-red-600 dark:text-red-400' : 'text-slate-700 dark:text-slate-300'">
                  {{ ((b.spent / b.amount) * 100).toFixed(0) }}%
                </span>
              </div>
              <div class="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-1.5">
                <div
                  class="h-1.5 rounded-full transition-all duration-500"
                  :class="b.spent > b.amount ? 'bg-red-500' : b.spent > b.amount * 0.8 ? 'bg-amber-500' : 'bg-primary-500'"
...
const loadData = () => {
  ...
  budgetAlerts.value = getBudgetAlerts(currentYear, currentMonth)
